...
- **SECURITY queue: one P1 OPENED, PROVEN, and deliberately NOT closed.** The enrollment re-pair gate is bypassable: `/api/v1/pairing/start` is pre-auth and returns the pairing code to the same caller, and `/api/v1/pairing/complete` sets `config.token` to a caller-chosen value, so an unauthenticated caller that can reach the agent port installs a credential and then satisfies the gate's live-credential branch (reproduced end to end: control refused 403, bypass succeeded 200 and rebound the record fingerprint with caller-chosen scopes). **Owner decision required** — the fix is an authorization rule on the pairing surface itself (e.g. an already-enrolled agent accepts a new pairing only from loopback / an owner-authenticated desktop request), which changes V2-A pairing semantics and must keep the pair-then-enroll repair flow working. Do NOT treat the shipped gate as closing this.
