...
- [ ] Create a useful home dashboard with node health, storage pressure, running apps, game servers, failed jobs and actionable alerts. *(Actionable alerts are reachable since cycle 14 — the Notifications page lists and acknowledges them; the remaining gap is the dashboard's own consolidation of these signals.)*
...
**Status:** Implementation: Partial — container lifecycle, policy gate, volume protection and preflight shipped; controlled image rollback is an open owner decision (bullet 6). Acceptance: PARTIAL — the Docker engine gate was executed and passed in cycle 25 against the real socket (`docker:smoke`: Docker API OK), but the wider destructive list (bad image, occupied port, missing volume, failed update) has no single harness and was not run. Evidence: `docker:smoke`, `docker:capabilities:smoke`, `docker:policy:smoke`, `docker:hostile-input:smoke`, `docker:job-lifecycle:smoke`, `docker:ipc-authorization:smoke`, `marketplace:provider-disk-preflight:smoke`; policy gate at `src/shared/dockerPolicy.js`.
...
**Status:** Implementation: Partial — package metadata, install transactions, runtime pins, the install-plan preview, the publisher-trust warning and catalog export/import ship (bullets 5, 7–8; see their annotations for the honest limits). Acceptance: NOT VERIFIED — clean-host reproducible install queued. Evidence: `marketplace:template-metadata:smoke`, `marketplace:archive-safety:smoke`, `marketplace:install-transaction:smoke`, `runtime:pin-guard:smoke`, `dependencies:smoke`, `publisher-trust:smoke`, `publisher-trust:display:smoke`, `catalog-transfer:smoke`, `catalog-transfer-ipc-exposure:smoke`; install-plan renderer at `app.js:15121`.
...
- [ ] Provide a bounded extension/adaptor interface and a clear trust warning for third-party executable content. *(Cycle 18: the publisher-trust warning renders in the app from the policy's own exported operator copy (`app.js` `createPublisherTrustNotice`, covered by `publisher-trust:display:smoke`) and fails closed. A bounded extension/adaptor interface is still not built, so the bullet stays open.)*
- [x] Support catalog export/import and document offline installation limits. *(Cycle 16: `src/services/catalogTransferService.js` — schema-versioned, byte-stable/diffable export, import with explicit duplicate (first wins) and conflict (never overwrite) rules, refuses a newer schema and an unjustified `verified` declaration, bounded document/entry sizes, and the seven offline limits returned in every import result. Cycle 17 wired `marketplace:exportCatalog|importCatalog` with preload exposure (`catalog-transfer-ipc-exposure:smoke`), and the renderer surface ships (cycle 25). **Honest limit:** import validates and reports without persisting — no write channel exists.)*
...
**Status:** Implementation: Partial — player information, scheduled restarts with warnings, per-game world backup scopes, and the game-adapter contract (documentation + machine-checkable validator + drift gate) shipped; FiveM's remaining live gates are outstanding and the contract records ten divergences rather than smoothing them (bullet 1). Acceptance: NOT VERIFIED — two-instance live drill queued. Evidence: `minecraft:players:smoke`, `restart:schedule:smoke`, `backups:world-scopes:smoke`, `backups:consistency:smoke`, `game-adapter-contract:smoke`, `permission-matrix:smoke`.
...
- [ ] Support game-specific configuration, mods/plugins and version changes only through declared adapter capabilities. *(The contract ships (bullet 1), but the enforcement gap is real and recorded: only SteamCMD adapters have an in-place update — Minecraft, FiveM and Terraria have none — four recognised families return `UNSUPPORTED_GAME_CONFIG`, and readiness is one shared regex rather than a per-adapter capability.)*
...
**Status:** Implementation: Implemented (code-complete) — node lifecycle, capability-aware upgrades, fleet aggregation/batch, offline job policy and workload transfer all shipped. Acceptance: PARTIAL — the two-node drill was executed and passed in cycle 25 (`multi-node:fleet:smoke` spawns two real Agent processes on two ports; seven legs pass), with the transfer leg and per-OS legs still recorded as not live. Evidence: `node:lifecycle:smoke`, `agent:self-update:smoke`, `agent:compatibility:smoke`, `fleet:aggregation:smoke`, `multi-node:fleet:smoke`, `node:stale-response:smoke`, `instances:job-expiry:smoke`, `workload:transfer:smoke`, `workload:transfer-gate:smoke`.
...
**Status:** Implementation: Partial — permission-scoped shell, network inventory, firewall lifecycle, reverse-proxy/certificate lifecycle (state only; no activation or issuance) and tunnel/public-access providers ship. Acceptance: NOT VERIFIED — publish/revoke drill queued. Evidence: `ssh:lifecycle:smoke`, `ssh:redaction:smoke`, `ssh:ipc-authorization:smoke`, `network-inventory:smoke`, `firewall:lifecycle:smoke`, `public-access:smoke`, `public-access:create-service:smoke`, `reverse-proxy:smoke`, `reverse-proxy:surface:behavior:smoke`.
...
- [x] Add service domains, reverse proxy routes and certificate lifecycle for supported web workloads. *(Cycle 16: `src/shared/reverseProxyPolicy.js` + `agent/src/services/reverseProxyService.js` — fail-closed route validation (hostname/path/upstream/protocol/TLS), loopback-only upstream by default, wildcard only by explicit grant, atomic bounded registry, real TCP upstream reachability, and a certificate lifecycle state machine that reports `unknown` (never valid) when expiry is unreadable; reachable through two permission-tiered public-access endpoints. Cycle 18 added the renderer card (new desktop channels + preload exposure) which displays an unreadable certificate as `unknown`, managed TLS as `pending`, and `applied:false` as NOT applied. **Honest limit — state, not service:** no proxy configuration is written (every apply returns `applied:false` with `REVERSE_PROXY_ACTIVATION_UNAVAILABLE`), no certificate is issued (`managed` stays `pending`), and DNS is not touched.)*
- [x] Support selected tunnel/public-access providers through explicit provisioning, readiness, failure and cleanup states.
- [x] Distinguish HTTP routing from game-server TCP/UDP requirements; show actual provider and protocol limits. *(Cycle 18: `src/shared/exposureRequirementsPolicy.js` classifies an exposure request into protocol, transport and workload kind, selects the mechanism (HTTP routing vs raw port exposure), and **refuses rather than falls back** — HTTP routing for a UDP workload is `EXPOSURE_MECHANISM_UNSATISFIED` with no port-forward substitution. Provider limits are DECLARED data: every quantitative limit (max hostnames, concurrent tunnels, bandwidth) is `value: null, state: "unknown"` because no provider was contacted, and a disabled provider reports `unknown` rather than `unsupported`. A drift guard fails the smoke if a capability boolean flips without the table.)*
...
- [x] Surface DNS/certificate/provider errors and residual billable resources where applicable; never imply provisioning succeeded from request acceptance alone. *(Cycle 18: a diagnostics snapshot reports per workload the DNS error, certificate error and provider error from recorded data, with a top-level provisioning state of `known-succeeded` / `known-failed` / **`unknown`**, and the rule enforced in code that an unestablished state is never rendered as success. A certificate with an unreadable expiry surfaces `unknown` with `EXPIRY_UNDETERMINABLE`; managed TLS with no record stays `pending`. Residual billable resources default to `state: "unknown", count: null, detectable: false` and **cannot report zero** unless a detector covered every workload — a partial detector still reports `null`. **Honest limits:** no provider was contacted, so all limits are declared data, and nothing writes the diagnostics file yet, so the error-surfacing path is exercised only through a fixture.)*
...
**Status:** Implementation: Partial — the role/resource-scope permission model, scoped Agent tokens, secret redaction, workload trust levels, resource-limit enforcement, package-provenance review, audit retention/review/export and the threat model ship (bullets 3–8, each annotated with its honest limits). Session revocation/credential rotation/protected recovery remain partial (bullet 2). Acceptance: NOT VERIFIED — the actor×resource×action live drill is queued (the hermetic `permission-matrix:smoke` harness exists). Evidence: `permission-matrix:smoke`, `agent-scope-enforcement:smoke`, `authority:permissions:smoke`, `redaction:smoke`, `agent:token:smoke`, `ssh:ipc-authorization:smoke`, `backups:ipc-authorization:smoke`, `audit-access-ipc-exposure:smoke`.
...
- [x] Review package provenance, update trust, extension execution and dependency vulnerability handling. *(Cycle 18: `src/shared/dependencyProvenancePolicy.js` reuses the publisher-trust vocabulary rather than inventing a second trust idiom, fails closed (a lockfile hash is a DECLARED hash, never an established match), refuses an update as safe when no integrity fact exists, and denies extension execution by default with a grant that cannot lift a trust refusal. `src/services/dependencyService.js` reports this repository's real dependencies with provenance and advisory status. **The honest result:** all 10 direct dependencies are `unverified-publisher` and **`clean: 0` is a hard invariant** — no code path can emit "advisory-free". Advisory data comes from `docs/v2/SECURITY_TRIAGE_RECORD.md` (a point-in-time triage, not a feed, `feed: null`), one dependency has a recorded mitigated advisory with GHSA ids, and the other nine report `unknown` because absence from the triage record is not a clean bill of health. **Honest limits:** nothing hashes installed bytes, so no dependency can reach `verified`; the report is read-only and points at this application tree, never the selected node's host dependencies (`dependencies:getProvenanceReport` in `src/ipc/dependenciesIpc.js`, rendered on the Agent Control page); and no package in this repository can be called free of advisories.)*
- [x] Provide audit retention, access review and export with sensitive data redaction. *(Cycle 16: `src/shared/auditRetentionPolicy.js` + `src/services/securityService.js` — a retention decision rule that prunes only an explicit routine allowlist and **refuses the whole operation** rather than touching a protected class to satisfy a size cap (fail-closed: anything unclassifiable is protected); a windowed access-review projection; and a bounded, redacted, byte-identical-diffable export. Cycles 17–18 wired the three functions: permission-gated IPC (`security:getAuditRetentionReport`, `security:getAuditAccessReview`, `security:exportAuditWindow` in `src/ipc/securityIpc.js`) and a Security-page audit section that loads the retention decision, the access review, and a redacted export preview. **Honest limit:** retention is a decision, not enforcement — this build never rewrites the audit log, because deleting audit data is an owner-approval action.)*
...
**Status:** Implementation: Partial — health unification, correlation IDs, redacted diagnostics, controlled updates and (since cycle 14) alert surfacing ship; schema rollback is not implemented (bullet 6) and the RTO/RPO targets and drill results are outstanding (bullet 8). Acceptance: NOT VERIFIED — induced-failure drill queued; the alert panel rendered at runtime in the cycle-19/20 acceptance passes, but no induced failure has been exercised. Evidence: `dashboard:metrics:smoke`, `node-health:smoke`, `instances:health-summary:smoke`, `correlation-id:smoke`, `diagnostics:smoke`, `redaction:smoke`, `updates:download-safety:smoke`, `agent:self-update:smoke`, `agent:compatibility:smoke`, `alert-engine:smoke`, `preload-exposure-contract:smoke`.
...
- [x] Provide alerts for disk pressure, failure, offline agents, backup age and resource exhaustion with deduplication and quiet recovery behavior. *(Was unreachable at the audited SHA; wired in cycle 14 — the Notifications page lists active alerts and acknowledges them, with the preload exposure covered behaviorally by `preload-exposure-contract:smoke`. The panel rendered at runtime in the cycle-19/20 acceptance passes; an induced failure has still not been exercised — see the acceptance line above.)*
