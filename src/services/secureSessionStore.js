...
class SecureSessionStore {
  ...
  read() {
    ...
    if (schemaVersion < SECURE_SESSION_SCHEMA_VERSION) {
      ...
      const originalBytes = fs.readFileSync(this.filePath);
      const backupExisted = fs.existsSync(backupPath);
      if (!backupExisted) fs.copyFileSync(this.filePath, backupPath, fs.constants.COPYFILE_EXCL);
      // V2-J bullet 6: the envelope rewrite below only runs behind a recovery
      // point that was read back and verified. An unverified copy is refused,
      // not warned past. The encrypted session payload cannot be rebuilt from
      // anywhere else.
      ...
      const recoveryVerdict = decideMigrationRecovery({
        storeId: "secure-session",
        fromSchemaVersion: schemaVersion,
        toSchemaVersion: SECURE_SESSION_SCHEMA_VERSION,
      ...
      if (recoveryVerdict.verdict !== "proceed") {
        ...
        throw secureSessionError(
          "SECURE_SESSION_MIGRATION_RECOVERY_UNVERIFIED",
          "Encrypted session state could not be migrated safely because the pre-migration recovery point could not be verified.",
          this.filePath,
          null,
          { storeId: "secure-session", reason: recoveryVerdict.reason, verification: recoveryVerification.reason },
        );
      }
