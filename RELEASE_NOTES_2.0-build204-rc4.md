...
- The packaged Agent runtime now bundles its full dependency closure
  (js-yaml/ssh2), fixing the 2.0-build204-rc3 real-host acceptance failure in
  which the packaged Agent died at startup with `MODULE_NOT_FOUND`. The rc3
  artifacts are broken and must not be installed.
