...
assert(functionBody(appSource, "refreshPlayitStatus").includes('document.querySelector("[data-public-access-loading]")?.setAttribute("hidden", "")'), "Public Access refresh must clear stale grouped loading state.");
assert(functionBody(appSource, "refreshPlayitStatus").includes("refreshPlayitManagement(payload, requestContext).catch"), "Optional Playit management refresh must not block the primary Public Access snapshot.");
assert(functionBody(appSource, "refreshPlayitStatus").includes("const isSelectedNodeRequest = () => requestContext.nodeId === getSelectedNodeId()"), "Public Access snapshots must survive background reconciliation when the selected node identity is unchanged.");
assert(!/finally\s*\{\s*if\s*\(isNodeRequestCurrent\(requestContext\)\)/.test(functionBody(appSource, "refreshPlayitStatus")), "Public Access request cleanup must not depend on a stale node generation.");
